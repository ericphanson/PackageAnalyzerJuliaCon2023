...
using PackageAnalyzer, DataFrames, Legolas

# ╔═╡ 1b22be81-fa7c-4538-9bb8-a248f86a6169
using LicenseCheck

# ╔═╡ e922319c-2603-45d3-8ccf-9c825b03efb9
using Scratch, PlutoUI

# ╔═╡ f05a11ff-94cd-4364-a257-d2b211a5b98f
...
md"## Demo: Comparing an individual package against General"

# ╔═╡ 04fdc396-b52d-43be-a907-553747f75f01
@bind name confirm(TextField(; default="PackageAnalyzer"))

# ╔═╡ a6641b6a-8e2c-4eb9-b286-39aa323b959f
md"## Analyzing the dependencies of an application"

# ╔═╡ d4d52621-b968-4fcf-8251-0f055c72b22b
md"## Analyzing a manifest.toml"

# ╔═╡ 4c001622-1d66-4797-9800-017294f9d367
md"""
* A `Manifest.toml` file records the names, UUIDs, and versions of all dependencies and transitive dependencies (dependencies of dependencies)
* This provides a fixed set of versions for us to analyze
"""
...
# ╔═╡ bd710929-89e8-4934-97ea-e1082ec9d1b6
auth = PackageAnalyzer.GitHub.AnonymousAuth()

# ╔═╡ 0146ca08-e186-4103-95d3-dd11399c6404
m = DataFrame(analyze_manifest("fixed_manifest.toml"; auth))
...
# ╔═╡ 462551b3-c2c5-4aef-8b8c-f77dfb604555
begin
	using CairoMakie
...
# ╔═╡ 0f0adae6-830f-40c5-a32d-95ce28664625
function loghist(v; kwargs...)
    fig = Figure()
    ax = Axis(fig[1, 1]; kwargs...)
    h = loghist!(ax, v)
    return Makie.FigureAxisPlot(fig, ax, h)
end
...
function sum_docs(table)
    return sum(x.code + x.comments
               for x in table
               if x.directory in ("docs", "doc") && !in(x.language, rm_langs) &&
                  !in(x.sublanguage, rm_langs); init=0)
end

# ╔═╡ 415bfc63-3c09-49ae-a1e0-51ce34300925
function sum_docstrings(table)
    return sum(x.docstrings
               for x in table
...
begin
	...
	transform!(all_packages,
                ...
                :lines_of_code => ByRow(sum_docs) => :lines_of_docs,
				:lines_of_code => ByRow(sum_docstrings) => :lines_of_docstrings)

	ci_cols = ["github_actions", "travis", "appveyor", "cirrus", "circle", "drone", "buildkite", "azure_pipelines", "gitlab_pipeline"]
	transform!(all_packages, AsTable(ci_cols) => ByRow(any) => :any_CI,
	:subdir => ByRow(!isempty) => :is_subdir,
	:name => ByRow(endswith("jll")) => :is_jll,
	[:lines_of_julia_src_code, :lines_of_julia_test_code] => ByRow((src, test) -> (test / (src + test))) => :test_fraction_of_src_plus_test,
	:license_files => ByRow(
		# A somewhat weak check, but consistent w/ RegistryCI:
		# We can find some file that has some OSI-approved license in it
		# (even if the same file has some non-OSI-approved licenses as well)
...
grps = groupby(versions_with_data, [:name, :date]);

# ╔═╡ c0d481ac-8578-4ce4-88ec-7f991f7a76a5
row = let
	key = (; name, date=analysis_date)
	only(get(grps, key, [NamedTuple()]))
end;
...
# ╔═╡ 7df7ab3c-22c9-4cdc-af45-d6f9a7087d4c
...
let
	...
	# x_dates = 	AoG.optimal_datetime_range((all_dates[1], all_dates[end]))
...
let
    fig = Figure()
        _plt(fig[1, 1], :lines_of_julia_src_code; x_name="Julia `src` code")
        _plt(fig[1, 2], :lines_of_julia_test_code; x_name="Julia `test` code")

        _plt(fig[2, 1], :lines_of_docs_plus_readme; x_name="Docs+README")
        _plt(fig[2, 2], :lines_of_docstrings; x_name="Docstrings")
    fig
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "c25a2e4bb3c54d793a0842b33daf124aa502e385"
...
[[deps.Legolas]]
deps = ["Arrow", "ConstructionBase", "Tables", "UUIDs"]
git-tree-sha1 = "2af55bc396c8ee085d0953e48308791f247064f5"
...
git-tree-sha1 = "59cd04be7b28f02c8d2f00aabc19c898f9cd2315"
